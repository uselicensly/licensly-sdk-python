...
#   2. Sibling ../api (superproject checkout) — pack from normative api/
#   3. GitHub release asset contract-vVERSION on uselicensly/licensly
#      (needs network; private repo requires GH_TOKEN / LICENSLY_CONTRACT_TOKEN)
#
...
if [[ -n "${LICENSLY_CONTRACT_BUNDLE:-}" ]]; then
  ...
  # Running as a submodule of licensly.
  pack_from_api "$(cd "${ROOT}/.." && pwd)"
elif [[ -d "${ROOT}/api" && -f "${ROOT}/scripts/pack-contract-bundle.py" ]]; then
  # Running inside the parent repo itself.
  pack_from_api "$ROOT"
else
  TOKEN="${LICENSLY_CONTRACT_TOKEN:-${GH_TOKEN:-${GITHUB_TOKEN:-}}}"
  TAG="contract-v${VERSION}"
  URL="https://github.com/uselicensly/licensly/releases/download/${TAG}/${ARCHIVE_NAME}"
  echo "Downloading $URL"
  if [[ -n "$TOKEN" ]]; then
    curl -fsSL -H "Authorization: Bearer ${TOKEN}" -H "Accept: application/octet-stream" \
      -o "$TMP/$ARCHIVE_NAME" "$URL"
  else
...
if [[ "$GOT" != "$EXPECT" ]]; then
  ...
  echo "  got      $GOT" >&2
  echo "Update contract.lock.json together with the parent contract release." >&2
  exit 1
fi
